...
if [ -z "${OS_ID##*debian*}" ]; then
  ...
  $sudo yum update
  $sudo yum -y install python3 pkgconfig socat libffi-devel gmp-devel openssl-devel ncurses-libs systemd-devel zlib-devel npm yarn make gcc-c++ tmux git wget epel-release jq
...
curl -s -o sendADA.sh https://github.com/cardano-community/guild-operators/blob/master/scripts/cnode-helper-scripts/sendADA.sh
