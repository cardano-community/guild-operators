...
[[ ${current_slot_in_epoch} -ge ${min_slot} ]] &&
  next_epoch_nonce=$(echo "$(${CCLI_PATH} query protocol-state --testnet-magic "${NWMAGIC}" | jq -r .candidateNonce.contents)$(${CCLI_PATH} query protocol-state --testnet-magic "${NWMAGIC}" | jq -r .lastEpochBlockNonce.contents)" | xxd -r -p | b2sum -b -l 256 | awk '{print $1}') &&
