...
getNodeMetrics() {
  ...
  blocknum=${node_metrics_arr[0]}
  epochnum=${node_metrics_arr[1]}
  slot_in_epoch=${node_metrics_arr[2]}
  slotnum=${node_metrics_arr[3]}
  [[ ${node_metrics_arr[4]} != '-' ]] && density=$(bc <<<"scale=3;$(printf '%3.5f' "${node_metrics_arr[4]}")*100/1") || density=0.0
  tx_processed=${node_metrics_arr[5]}
  mempool_tx=${node_metrics_arr[6]}
  mempool_bytes=${node_metrics_arr[7]}
  kesperiod=${node_metrics_arr[8]}
  remaining_kes_periods=${node_metrics_arr[9]}
  isleader=${node_metrics_arr[10]}
  adopted=${node_metrics_arr[11]}
  didntadopt=${node_metrics_arr[12]}
  about_to_lead=${node_metrics_arr[13]}
  nodeStartTime=${node_metrics_arr[14]}
...
function get-metrics() {
  ...
  cpuutil=$(awk -v a="$(
    ...
    sleep 1
  )" '/cpu /{split(a,b," "); print 100*($2+$4-b[1])/($2+$4+$5-b[2])}' /proc/stat)
  ...
  for metric_var_name in $(env | grep ^METRIC | sort | awk -F= '{print $1}'); do
