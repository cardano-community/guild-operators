DROP FUNCTION IF EXISTS koios.account_assets (text);

CREATE FUNCTION koios.account_assets (_address text DEFAULT NULL)
  RETURNS TABLE (
    asset_policy text,
    asset_name text,
    quantity numeric)
...
COMMENT ON FUNCTION koios.account_assets IS 'Get the native asset balance of an account';
