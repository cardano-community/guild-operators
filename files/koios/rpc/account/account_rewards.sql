DROP FUNCTION IF EXISTS koios.account_rewards (text, numeric);

CREATE FUNCTION koios.account_rewards (_stake_address text, _epoch_no numeric DEFAULT NULL)
  RETURNS TABLE (
    earned_epoch bigint,
...
COMMENT ON FUNCTION koios.account_rewards IS 'Get the full rewards history (including MIR) for a stake address, or certain epoch if specified';
