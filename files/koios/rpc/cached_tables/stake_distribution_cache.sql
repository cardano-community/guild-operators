CREATE TABLE IF NOT EXISTS koios.STAKE_DISTRIBUTION_CACHE (
...
CREATE TABLE IF NOT EXISTS koios.CONTROL_TABLE (
  key text PRIMARY KEY,
  last_value text NOT NULL,
  artifacts text
);

DROP PROCEDURE IF EXISTS koios.UPDATE_STAKE_DISTRIBUTION_CACHE ();

DROP FUNCTION IF EXISTS koios.UPDATE_STAKE_DISTRIBUTION_CACHE ();

CREATE PROCEDURE koios.UPDATE_STAKE_DISTRIBUTION_CACHE ()
LANGUAGE PLPGSQL
AS $$
DECLARE
...
BEGIN
  ...
  INSERT INTO koios.STAKE_DISTRIBUTION_CACHE
  SELECT
    STAKE_ADDRESS,
    POOL_ID,
...
ON CONFLICT (STAKE_ADDRESS)
  ...
  INSERT INTO koios.CONTROL_TABLE (key, last_value)
...
DROP FUNCTION IF EXISTS koios.UPDATE_STAKE_DISTRIBUTION_CACHE_CHECK CASCADE;

CREATE FUNCTION koios.UPDATE_STAKE_DISTRIBUTION_CACHE_CHECK ()
  RETURNS VOID
  LANGUAGE PLPGSQL
  AS $$
DECLARE
...
BEGIN
  ...
  IF _last_update_block_diff >= 185 THEN
    ...
    CALL koios.UPDATE_STAKE_DISTRIBUTION_CACHE ();
  END IF;
  RETURN;
END;
$$;
...
SELECT
  koios.UPDATE_STAKE_DISTRIBUTION_CACHE_CHECK ();

CREATE INDEX IF NOT EXISTS idx_pool_id ON koios.STAKE_DISTRIBUTION_CACHE (POOL_ID);
