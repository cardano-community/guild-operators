...
######## Functions ########
  ...
  deploy_query_updates() {
    ...
    if ! rpc_file_list=$(curl -s -f -m ${CURL_TIMEOUT} https://api.github.com/repos/${G_REPO}/contents/files/grest/rpc?ref=${BRANCH} 2>&1); then
    ...
    for row in $(jq -r '.[] | @base64' <<<${rpc_file_list}); do
      if [[ $(jqDecode '.type' "${row}") = 'dir' ]]; then
        ...
        if ! rpc_file_list_subdir=$(curl -s -m ${CURL_TIMEOUT} "https://api.github.com/repos/${G_REPO}/contents/files/grest/rpc/$(jqDecode '.name' "${row}")?ref=${BRANCH}"); then
