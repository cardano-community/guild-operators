...
#LEIOS_NODE_VERSION='latest'    # Leios prototype release tag used for NETWORK=musashi with -s d. Set to 'latest' to resolve newest release.
######################################
...
usage() {
  cat <<-EOF >&2
		...
		Usage: $(basename "$0") [-n <mainnet|guild|preprod|preview|musashi>] [-p path] [-t <name>] [-b <branch>] [-u] [-s [p][b][l][m][d][c][o][w][x][f][s]]
		...
		-s    Selective Install, only deploy specific components as below:
		  ...
		  d   Download latest (released) binaries for bech32, cardano-address, cardano-node, cardano-cli, cardano-db-sync and cardano-submit-api; for musashi, download Leios cardano-node/cardano-cli only (Default: skip)
...
set_defaults() {
  ...
  [[ "${NETWORK}" == "musashi" && -z "${LEIOS_NODE_VERSION}" ]] && LEIOS_NODE_VERSION="latest"
...
populate_cnode() {
  ...
  if [[ ${NETWORK} =~ ^(mainnet|preprod|preview|guild|musashi)$ ]]; then
    ...
    err_exit "Unknown network specified! Kindly re-check the network name, valid options are: mainnet, guild, preprod, preview, or musashi."
  fi
  ...
  [[ ${NETWORK} == "musashi" && ( ! -f dijkstra-genesis.json || ! -f peer-snapshot.json ) ]] && missing_config="Y"
  ...
  if [[ ${FORCE_OVERWRITE} = 'Y' || ${missing_config} = 'Y' ]]; then
    ...
    [[ ${NETWORK} == "musashi" ]] && mv -f dijkstra-genesis.json.tmp dijkstra-genesis.json
    [[ ${NETWORK} == "musashi" ]] && mv -f peer-snapshot.json.tmp peer-snapshot.json
