...
if [ "$WANT_BUILD_DEPS" = 'Y' ]; then
  ...
  if [ -z "${OS_ID##*debian*}" ]; then
    ...
    $sudo apt-get -y install libpq-dev python3 build-essential pkg-config libffi-dev libgmp-dev libssl-dev libtinfo-dev systemd libsystemd-dev libsodium-dev zlib1g-dev yarn make g++ tmux git jq wget libncursesw5 gnupg aptitude libtool autoconf secure-delete net-tools tcptraceroute > /dev/null;rc=$?
    if [ $rc != 0 ]; then
    ...
    echo "  Installing missing prerequisite packages, if any.."
    $sudo yum -y install python3 coreutils pkgconfig libffi-devel gmp-devel openssl-devel ncurses-libs ncurses-compat-libs systemd systemd-devel libsodium-devel zlib-devel npm yarn make gcc-c++ tmux git wget epel-release jq gnupg libtool autoconf srm net-tools tcptraceroute > /dev/null;rc=$?
    if [ $rc != 0 ]; then
      echo "An error occurred while installing the prerequisite packages, please investigate by using the command below:"
