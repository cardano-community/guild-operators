...
if [ "$WANT_BUILD_DEPS" = 'Y' ]; then
  ...
  if [[ "${OS_ID}" =~ ebian ]] || [[ "${DISTRO}" =~ ebian ]]; then
    ...
    $sudo NEEDRESTART_MODE=a DEBIAN_FRONTEND=noninteractive DEBIAN_PRIORITY=critical apt-get ${pkg_opts} update > /dev/null
    ...
    $sudo NEEDRESTART_MODE=a DEBIAN_FRONTEND=noninteractive DEBIAN_PRIORITY=critical apt-get ${pkg_opts} install ${pkg_list} > /dev/null;rc=$?
    if [ $rc != 0 ]; then
