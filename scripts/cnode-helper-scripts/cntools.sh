  ...
  while true; do
    ...
    if [[ "${PROTECT_KEYS}" = "yes" ]]; then
      [[ ! -f "${staking_sk_file}.gpg" ]] && {
        say "${RED}ERROR${NC}: 'PROTECT_KEYS=yes' but no gpg encrypted file found on disk (are you sure this is a staking wallet?):" "log"
      ...
      getPassword # $password variable populated by getPassword function
      if ! decryptFile "${staking_sk_file}.gpg" "${password}"; then
        say "${RED}ERROR${NC}: Unable to decrypt staking sk file:" "log"
        say "${staking_sk_file}.gpg" "log"
        unset password
        exit 1
      else
        if [[ ! -f "${staking_sk_file}" ]]; then
          say "${RED}ERROR${NC}: staking wallet signing key file not found:" "log"
          say "${staking_sk_file}" "log"
          echo "" && read -r -n 1 -s -p "press any key to return to home menu" && continue
        fi
    ...
    if [[ "${PROTECT_KEYS}" = "yes" ]]; then
      ...
      echo ""
      if ! decryptFile "${staking_vk_file}.gpg" "${password}"; then
        say "${RED}ERROR${NC}: 'Unable to decrypt staking vk file:" "log"
        say "${staking_vk_file}.gpg" "log"
        unset password
        exit 1
      else
        if [[ ! -f "${staking_vk_file}" ]]; then
          say "${RED}ERROR${NC}: staking wallet verification key file not found:" "log"
          say "${staking_vk_file}" "log"
          echo "" && read -r -n 1 -s -p "press any key to return to home menu" && continue
        fi
    ...
    if [[ "${PROTECT_KEYS}" = "yes" ]]; then
      ...
      if ! decryptFile "${pay_payment_sk_file}.gpg" "${password}"; then
        say "${RED}ERROR${NC}: Unable to decrypt payment sk file:" "log"
        say "${pay_payment_sk_file}.gpg" "log"
        unset password
        exit 1
      else
