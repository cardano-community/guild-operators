...
while true; do
  ...
  if [[ ${check_peers} = "true" ]]; then
    ...
    printf "${VL} If CNCLI is not activated blocks created is" && tput cup ${line} ${width} && printf "${VL}\n" && ((line++))
    printf "${VL} taken from EKG metrics. Invalid, Missed, Ghosted" && tput cup ${line} ${width} && printf "${VL}\n" && ((line++))
