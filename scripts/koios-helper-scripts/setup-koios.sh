...
######## Functions ########
  ...
  update_check() {
    ...
    if [[ ${SKIP_UPDATE} != Y ]]; then
      ...
      checkUpdate setup-koios.sh ${ENV_UPDATED}
    fi
  ...
  get_cron_job_executable() {
    local job=$1
    local job_url="${URL_RAW}/files/koios/cron/jobs/${job}.sh"
  ...
  populate_genesis_table() {
    ...
    psql "${PGDATABASE}" <<-SQL >/dev/null
			...
			BEGIN;
			DROP TABLE IF EXISTS koios.genesis;
			CREATE TABLE koios.genesis (
			...
			COMMIT;
			SQL
    psql "${PGDATABASE}" -c "INSERT INTO koios.genesis VALUES ( '${SHGENESIS[4]}', '${SHGENESIS[2]}', '${SHGENESIS[0]}', '${SHGENESIS[1]}', '${SHGENESIS[3]}', '${SHGENESIS[5]}', '${SHGENESIS[6]}', '${SHGENESIS[7]}', '${SHGENESIS[8]}', '${SHGENESIS[9]}', '${SHGENESIS[10]}', '${ALGENESIS}' );" > /dev/null
  }
  deploy_postgrest() {
  ...
  deploy_haproxy() {
    ...
    checkUpdate koios-poll.sh Y N N koios-helper-scripts >/dev/null
    checkUpdate checkstatus.sh Y N N koios-helper-scripts >/dev/null
  }
  deploy_monitoring_agents() {
    ...
    pushd "${CNODE_HOME}"/scripts >/dev/null || err_exit
    checkUpdate getmetrics.sh Y N N koios-helper-scripts >/dev/null
    echo -e "[Re]Installing Monitoring Agent.."
    e=!
    sudo bash -c "cat <<-EOF > ${CNODE_HOME}/scripts/koios-exporter.sh
			#${e}/usr/bin/env bash
			socat TCP-LISTEN:8059,reuseaddr,fork     SYSTEM:\"echo HTTP/1.1 200 OK;SERVED=true bash ${CNODE_HOME}/scripts/getmetrics.sh;\"
			EOF"
    sudo chown $USER:$USER "${CNODE_HOME}"/scripts/koios-exporter.sh
    chmod 755 "${CNODE_HOME}"/scripts/koios-exporter.sh
  }
  deploy_configs() {
    # Create PostgREST config template
    echo "[Re]Deploying Configs.."
    [[ -f "${CNODE_HOME}"/priv/pgrest.conf ]] && cp "${CNODE_HOME}"/priv/pgrest.conf "${CNODE_HOME}"/priv/pgrest.conf.bkp_$(date +%s)
    ...
    bash -c "cat <<-EOF > ${HAPROXY_CFG}
			...
			backend koios_core
			  ...
			  external-check command ${CNODE_HOME}/scripts/koios-poll.sh
  ...
  deploy_systemd() {
    ...
    command -v haproxy >/dev/null && sudo bash -c "cat <<-EOF > /etc/systemd/system/haproxy.service
			...
			WantedBy=multi-user.target
			EOF"
    echo -e "  Koios Exporter Service"
    [[ -f "${CNODE_HOME}"/scripts/koios-exporter.sh ]] && sudo bash -c "cat <<-EOF > /etc/systemd/system/koios_exporter.service
			[Unit]
			...
			ExecStart=/bin/bash -l -c \"exec ${CNODE_HOME}/scripts/koios-exporter.sh\"
			KillSignal=SIGINT
			SuccessExitStatus=143
			StandardOutput=syslog
			StandardError=syslog
			SyslogIdentifier=koios_exporter
			TimeoutStopSec=5
			KillMode=mixed
			...
			WantedBy=multi-user.target
			EOF"
    sudo systemctl daemon-reload && sudo systemctl enable postgrest.service haproxy.service koios_exporter.service >/dev/null 2>&1
  ...
  deploy_query_updates() {
    ...
    if ! rpc_file_list=$(curl -s -f -m ${CURL_TIMEOUT} https://api.github.com/repos/cardano-community/guild-operators/contents/files/koios/rpc?ref=${BRANCH} 2>&1); then
      err_exit "\e[31mERROR\e[0m: ${rpc_file_list}"
    fi

    echo -e "  Add koios schema if missing and grant usage for web_anon"
    psql "${PGDATABASE}" <<-EOF >/dev/null
			...
			EOF
    populate_genesis_table
    echo -e "  [Re]Deploying Koios objects to DBSync.."
    for row in $(jq -r '.[] | @base64' <<<${rpc_file_list}); do
      if [[ $(jqDecode '.type' "${row}") = 'dir' ]]; then
        ...
        if ! rpc_file_list_subdir=$(curl -s -m ${CURL_TIMEOUT} "https://api.github.com/repos/cardano-community/guild-operators/contents/files/koios/rpc/$(jqDecode '.name' "${row}")?ref=${BRANCH}"); then
