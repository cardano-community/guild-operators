...
function get-metrics() {
  ...
  koiosschsize=$(psql -d cexplorer -c "SELECT sum(pg_relation_size(quote_ident(schemaname) || '.' || quote_ident(tablename))::bigint) FROM pg_tables WHERE schemaname = 'koios'" | awk 'FNR == 3 {print $1 $2}')
  dbsize=$(( pubschsize + koiosschsize ))
  ...
  export METRIC_koiosschsize="${koiosschsize}"
