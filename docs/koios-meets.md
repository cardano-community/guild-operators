...
# Koios Meeting summaries

Thank you all for joining and contributing to the project :smile:

Below you can find a short summary of every Koios meeting held, both for logging purposes and for those who were not able to attend.
...
=== "26Aug2021"
    ...
    ### Transaction submission feature
      - a post endpoint separate from Koios ones (different port), proxy'd over haproxy using same health check script appended for node
...
=== "19Aug2021"
    ...
    - Proposition to move the Koios schema and tables required by the API to smaller instances that can be scaled more easily
    ...
    - Enhance Koios-poll to use arguments and seperate haproxy backends, allowing for test based reduction
...
=== "12Aug2021"
    ...
    ### ACTIONS
      ...
      - Add a control table in Koios schema to record block_hash used for last update, start time, and end time. This will act as a checkpoint for polling of queries that are not live (separate backend in haproxy)
...
=== "29Jul2021"
    ...
    ### ACTIONS
      ...
      - add project metadata (requirements) to Koios doc header in a checklist format that folks can use to ensure their setup is up-to-date with the current project state
...
=== "22Jul2021"
    ...
    1. Team
        ...
        - catch live stake distributions in a separate table (in our `koios` schema)
...
=== "15Jul2021"
    ...
    - Koios project could be helpful for pool peek
...
=== "25Jun2021"
    ...
    - Homer, Ola, Markus, Priyank and Damjan have all set up their dbsync + Koios endpoints against guild network and added to topology.
    ...
    1. Directory structure on the repo -> General agreement is to have anything related to db-sync/Koios separated from the current cnode-helper-scripts directory. We can finalise the end locations of files a bit later, for now intent should be to simply add them all to /files/dbsync folder. `prereqs.sh` addendum can be done once artifacts are finalised (added a Trello ticket for tracking).
    2. DNS/haproxy configurations:
      We have two options:
      a. controlled approach for endpoints - wherein there is a layer of haproxy that will load balance and ensure tip being in sync for individual providers (individuals can provide haproxy OR Koios instances).
