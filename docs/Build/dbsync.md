...
```bash
cd ~/git/cardano-db-sync
PGPASSFILE=$CNODE_HOME/priv/.pgpass cardano-db-sync-extended --config ./config/mainnet-config.yaml --socket-path $CNODE_HOME/sockets/node0.socket --schema-dir schema/
```
