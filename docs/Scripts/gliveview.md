...
- **Block propagation** - Last delay measures the duration between when the last block was produced and the node learned about it. Within 3s measures how many blocks out of the total had a delay less than 3s. A healthy node needs to stay above 95%. Late blocks are blocks whose delay is larger than 5s. A high number of late blocks and a low percentace of blocks within 3s means the connectivity of the node (peers in, peers out, ping times) needs to be checked.
